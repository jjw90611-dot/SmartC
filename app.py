...
def write_cell(cell, text, size=10, bold=False, align='center', v_align='center', bg=None):
    """셀에 텍스트를 쓰고 서식 지정"""
    cell.text = ''
    if bg:
        set_cell_bg(cell, bg)
...
def add_header_marker(doc):
    """페이지 상단의 'S-OIL: Company Use Only' 표시"""
    add_para(doc, 'S‑OIL: Company Use Only', size=9, align='right', space_after=8, color=(100, 100, 100))
...
HEADER_BG = 'D9E1F2'   # 옅은 하늘색 (헤더용)
SUBHEADER_BG = 'F2F2F2'  # 옅은 회색
...
def create_word_document(data):
    ...
    set_font(run, size=40, bold=True)
    ...
    # 헤더 (2행 병합 헤더)
    write_cell(t.cell(0, 0), '작업장소', size=10, bold=True, bg=HEADER_BG)
    write_cell(t.cell(0, 1), '공정명', size=10, bold=True, bg=HEADER_BG)
    write_cell(t.cell(0, 2), '질식·중독 발생 위험\n유해위험 물질', size=10, bold=True, bg=HEADER_BG)
    write_cell(t.cell(0, 3), '관리방안', size=10, bold=True, bg=HEADER_BG)
    write_cell(t.cell(0, 4), '', size=10, bold=True, bg=HEADER_BG)
    ...
    write_cell(t.cell(1, 3), '입조 전', size=10, bold=True, bg=HEADER_BG)
    write_cell(t.cell(1, 4), '입조 중', size=10, bold=True, bg=HEADER_BG)
    ...
    write_cell(t3.cell(9, 2),
        '많은 양의 화학물질을 섭취한 경우 의사의 진찰을 받을 것',
        size=9, align='left', v_align='top')
    ...
    for i, h in enumerate(['성 명', '연 락 처', 'S‑OIL 안전대응팀']):
    ...
    write_cell(t8.cell(1, 2), '052‑231‑2004', bold=True)
    for r in range(2, 6):
    ...
    roles_L = ['S‑OIL 공사감독', '관리감독자 1', '관리감독자 2', '감시인 1', '감시인 2', '감시인 3']
    roles_R = ['작업자 1', '작업자 2', '작업자 3', '작업자 4', '작업자 5', '작업자 6']
    for i in range(6):
        write_cell(t9.cell(i+1, 0), roles_L[i])
        write_cell(t9.cell(i+1, 1), '')
        write_cell(t9.cell(i+1, 2), '')
        write_cell(t9.cell(i+1, 3), roles_R[i])
        write_cell(t9.cell(i+1, 4), '')
        write_cell(t9.cell(i+1, 5), '')
    ...
    for i, h in enumerate(['공정명', '작업장소', '용적(㎥)', '환기시설\n모델명', '환기팬 용량\n(㎥/h)', '사용수량\n(EA)']):
        write_cell(t11.cell(0, i), h, bold=True, bg=HEADER_BG)
    row_vals = [data['process_name'], data['space_name'], str(data['volume']),
                data['fan_model'], str(data['fan_capacity']), str(data['fan_count'])]
    for i, v in enumerate(row_vals):
        write_cell(t11.cell(1, i), v)
    set_col_widths(t11, [2.5, 2.5, 2, 3, 3, 2.5])
    add_para(doc, '* 작업 전 – 체적의 10배 이상 환기 ,  작업 중 – 시간당 작업장소 체적의 20회 이상 환기', size=9, space_before=4)

    add_para(doc, '', size=8)
    add_para(doc, '(8) 환기시설 설치 및 작동 현황', size=11, bold=True, space_after=6)
    t12 = doc.add_table(rows=1, cols=2)
    t12.alignment = WD_TABLE_ALIGNMENT.CENTER
    write_cell(t12.cell(0, 0), '특이\n사항', bold=True, bg=HEADER_BG)
    write_cell(t12.cell(0, 1), '\n\n\n\n\n', align='left', v_align='top')
    set_col_widths(t12, [3, 12.5])
    set_row_height(t12.rows[0], 4.5)
    doc.add_page_break()
    ...
    write_cell(t13.cell(len(eval_items)+1, 0), '프로그램평가결과', bold=True, bg=SUBHEADER_BG)
    write_cell(t13.cell(len(eval_items)+1, 1), '', bg=SUBHEADER_BG)
    write_cell(t13.cell(len(eval_items)+1, 2), '판정수', bold=True, bg=SUBHEADER_BG)
    write_cell(t13.cell(len(eval_items)+1, 3), '', bg=SUBHEADER_BG)
    t13.cell(len(eval_items)+1, 0).merge(t13.cell(len(eval_items)+1, 1))
    ...
    for line in [
        ...
        '3) 작업에 종사하는 전체 근로자가 작업을 하고 있던 장소를 떠났다가 돌아와 다시 작업을',
    ]:
    ...
    t15 = doc.add_table(rows=2, cols=2)
    t15.alignment = WD_TABLE_ALIGNMENT.CENTER
    write_cell(t15.cell(0, 0), '측정지점', bold=True, bg=HEADER_BG)
    write_cell(t15.cell(0, 1),
        '○ 작업장소에 대해서 수직방향 및 수평방향으로 각각 3개소 이상\n'
        '○ 작업에 따라 근로자가 출입하는 장소로서 작업시 근로자의 호흡위치를 중심',
        size=10, align='left', v_align='top')
    write_cell(t15.cell(1, 0), '측정방법', bold=True, bg=HEADER_BG)
    write_cell(t15.cell(1, 1),
        ...
        size=10, align='left', v_align='top')
    set_col_widths(t15, [3, 12.5])

    add_para(doc, '', size=8)
    add_para(doc, '<작업장소 형태별 측정지점>', size=10, bold=True, align='center', space_after=4)
    add_para(doc, '(작업장소 형태별 측정지점 이미지 삽입)', size=9, align='center', color=(120, 120, 120))
    doc.add_page_break()
    ...
    add_para(doc,
        '밀폐공간 작업시 작업장소에서 적정한 공기가 유지되도록 환기를 실시한 후 작업을 하며, 작업공간 내에서 '
        '유해가스가 지속적으로 발생한 경우 (양수기 가동, 슬러지 제거작업 등)에는 계속적으로 환기를 실시한다.',
        size=10, space_after=8)

    add_para(doc, '■ 작업장소 용적', size=11, bold=True, space_after=6)
    t16 = doc.add_table(rows=2, cols=3)
    t16.alignment = WD_TABLE_ALIGNMENT.CENTER
    for i, h in enumerate(['공정명(Item)', '작업장소 용적(m³)', '비고']):
        write_cell(t16.cell(0, i), h, bold=True, bg=HEADER_BG)
    write_cell(t16.cell(1, 0), data['process_name'])
    write_cell(t16.cell(1, 1), f'{data["volume"]} m³')
    write_cell(t16.cell(1, 2), '')
    set_col_widths(t16, [5, 5, 5])

    add_para(doc, '', size=6)
    add_para(doc, '■ 환기팬 용량   * 아래 송풍기/배풍기 위치도에 표시 1,2,3', size=11, bold=True, space_after=6)
    t17 = doc.add_table(rows=3, cols=5)
    t17.alignment = WD_TABLE_ALIGNMENT.CENTER
    for i, h in enumerate(['구분', '모델명', '환기팬 용량\n(m³/h)', '수량\n(EA)', '장비관리담당자\n(협력업체)']):
        write_cell(t17.cell(0, i), h, bold=True, bg=HEADER_BG)
    write_cell(t17.cell(1, 0), '1')
    write_cell(t17.cell(1, 1), data['fan_model'])
    write_cell(t17.cell(1, 2), f'{data["fan_capacity"]} m³/h')
    write_cell(t17.cell(1, 3), str(data['fan_count']))
    write_cell(t17.cell(1, 4), '')
    write_cell(t17.cell(2, 0), '합산', bold=True, bg=SUBHEADER_BG)
    write_cell(t17.cell(2, 1), '', bg=SUBHEADER_BG)
    total_cap = data['fan_capacity'] * data['fan_count']
    write_cell(t17.cell(2, 2), f'{total_cap} m³/h', bold=True, bg=SUBHEADER_BG)
    write_cell(t17.cell(2, 3), str(data['fan_count']), bold=True, bg=SUBHEADER_BG)
    write_cell(t17.cell(2, 4), '', bg=SUBHEADER_BG)
    set_col_widths(t17, [1.8, 3, 3.5, 2, 5])

    add_para(doc, '', size=6)
    add_para(doc, '■ 환기팬 환기능력 계산 (예시)', size=11, bold=True, space_after=4)
    add_para(doc, '● 작업시작 전 (작업장소 용적의 10배이상 환기)', size=10, bold=True, space_after=4)
    add_para(doc, '* KOSHA GUIDE 환기기준', size=10, bold=True, space_before=4, space_after=2)
    add_para(doc, '   작업 전 : 체적의 10배 이상 환기', size=10, space_after=2)
    add_para(doc, '   작업 중 : 시간당 작업장소 체적의 20회 이상 환기', size=10)
    doc.add_page_break()

    # ---------------- Page 13 : 환기 계산 ----------------
    add_header_marker(doc)
    vol = data['volume']
    fcap_min = round(total_cap / 60, 1)
    add_para(doc, f' ‑ 작업장소 용적(10배) : {vol}m³ × 10 = {vol*10:,} m³', size=10, space_after=2)
    add_para(doc, f' ‑ 환기팬 환기능력(m³/min) : 환기팬 ({data["fan_count"]}대) 환기능력은 {total_cap:,}m³/h = {fcap_min}m³/min', size=10, space_after=2)
    add_para(doc, f' ‑ 계산결과 : 환기팬({data["fan_count"]}대) 가동 시 환기능력이 {vol*10:,}m³ 조건을 만족함', size=10, space_after=8)

    add_para(doc, '● 작업중 (시간당 작업장소 용적의 20회 이상 환기)', size=10, bold=True, space_after=4)
    add_para(doc, f' ‑ 시간당 20회 환기 기준량 : {vol}m³ × 20회 = {vol*20:,} m³/hr', size=10, space_after=2)
    add_para(doc, f' ‑ 계산결과 : 환기팬({data["fan_count"]}대) 환기능력이 {total_cap:,}m³/h 으로 20회 환기 기준량({vol*20:,}m³)보다 크므로 만족함', size=10, space_after=8)

    add_para(doc, '<송풍기/배풍기 설치 위치도>', size=10, bold=True, align='center', space_after=4)
    add_para(doc, '(위치도 이미지 삽입)', size=9, align='center', color=(120, 120, 120), space_after=8)

    add_para(doc, '<작업장소에 따른 환기량>', size=10, bold=True, align='center', space_after=4)
    t18 = doc.add_table(rows=5, cols=2)
    t18.alignment = WD_TABLE_ALIGNMENT.CENTER
    write_cell(t18.cell(0, 0), '작업장소', bold=True, bg=HEADER_BG)
    write_cell(t18.cell(0, 1), '환기량', bold=True, bg=HEADER_BG)
    write_cell(t18.cell(1, 0), '잠함, 압기실 등의 압기 공법의 작업실', align='left')
    write_cell(t18.cell(2, 0), '피트 내부', align='left')
    write_cell(t18.cell(3, 0), '황화수소가 발생할 우려가 있는 탱크, 보일러 등의 내부', align='left')
    write_cell(t18.cell(4, 0), '탱크 내 퇴적물 제거작업 / 기타 밀폐공간', align='left')
    write_cell(t18.cell(1, 1),
        '작업 전 당해 밀폐공간 체적의 10배이상 환기하고 작업하는 동안 시간당 밀폐공간 체적의 20배이상 계속 환기한다.',
        align='left', v_align='center')
    for r in range(2, 5):
        write_cell(t18.cell(r, 1), '')
    t18.cell(1, 1).merge(t18.cell(4, 1))
    set_col_widths(t18, [6, 9.5])

    add_para(doc, '* 위험물 옥외탱크저장소의 경우 Tank 개방검사절차(SOM‑0‑580)를 준용한다.', size=9, space_before=6)
    add_para(doc, '<주의사항>', size=10, bold=True, space_before=6)
    add_para(doc, '○ 작업 전에는 산소 및 유해가스의 농도가 기준농도를 만족할 수 있도록 충분한 환기를 실시한다.', size=10)
    doc.add_page_break()
    ...
    t19 = doc.add_table(rows=2, cols=3)
    t19.alignment = WD_TABLE_ALIGNMENT.CENTER
    for i, h in enumerate(['구분', '이동식 송풍기', '송풍관']):
        write_cell(t19.cell(0, i), h, bold=True, bg=HEADER_BG)
    write_cell(t19.cell(1, 0), '점검\n사항', bold=True)
    write_cell(t19.cell(1, 1),
        '○ 전원코드의 단선 접속부의 접촉불량 유무\n'
        '○ 코드와 단자상과의 접속상태 불량 유무\n'
        '○ 코드의 끝에 “환기중․정지”등의 표지판 부착 유무',
        size=9, align='left', v_align='top')
    write_cell(t19.cell(1, 2),
        '○ 연소에 의한 구멍이나 파열유무\n'
        '○ 링, 나선의 손상유무\n'
        '○ 접속부의 확실한 고정여부',
        size=9, align='left', v_align='top')
    set_col_widths(t19, [2, 7, 6.5])

    add_para(doc, '', size=6)
    add_para(doc, '라. 보호구', size=11, bold=True, space_after=4)
    add_para(doc,
        '밀폐공간 작업시 유해가스에 의한 중독 및 질식에 의한 사고를 예방하기 위해 공기호흡기 및 송기마스크 등의 보호구를 반드시 착용한 상태에서 작업을 하고, '
        '사용시 사용장소 및 사용방법 등을 충분히 숙지한 후 사용한다. 다만, 작업시 보호구를 착용하는 것이 원칙이나 측정결과 등으로 밀폐공간 내에서의 작업이 안전하다고 판단될 경우 보호구를 착용하지 않아도 된다.',
        size=10, space_after=8)

    add_para(doc, '□ 공기호흡기', size=11, bold=True, space_after=4)
    add_para(doc, '1) 착용해야 할 장소', size=10, bold=True, space_after=2)
    add_para(doc,
        '밀폐장소 출입 작업시 다음과 같이 환기할 수 없거나 환기가 불충분한 경우로서 단기간 작업이 가능한 경우에는 공기호흡기를 반드시 착용하고 출입하며, '
        '고농도의 유기화합물 증기가 예상되는 경우 등에는 방독마스크를 착용하지 않는다.',
    ...
    add_para(doc, '', size=4)
    add_para(doc, '2) 공기호흡기의 점검사항 및 사용방법', size=10, bold=True, space_after=2)
    add_para(doc, '[사용전의 점검사항]', size=10, bold=True, space_after=2)
    add_para(doc, '① 봄베의 잔류압 검사', size=10)
    doc.add_page_break()

    # ---------------- Page 15 : 공기호흡기, 송기마스크, 안전보호구 ----------------
    add_header_marker(doc)
    for line in [
        '② 고압연결부의 검사',
    ...
    add_para(doc, '', size=4)
    add_para(doc, '□ 송기마스크', size=11, bold=True, space_after=4)
    add_para(doc,
    ...
    add_para(doc,
        '탱크나 맨홀과 같이 사다리를 사용하여 내부로 내려가야 하는 경우에는 안전대나 기타 구명밧줄 등을 사용하여 안전을 확보한다. '
        '비상시에 작업자를 피난시키거나 구출하기 위하여 안전대, 사다리, 구명밧줄 등 필요한 용구를 준비하고 이것의 사용방법을 작업자에게 숙지하도록 한다. '
        '작업시 작업장소에서 적정한 공기가 유지되도록 환기를 실시한 후 작업을 하며, 작업공간 내에서 유해가스가 지속적으로 발생한 경우(양',
        size=10, space_after=4)
    doc.add_page_break()

    # ---------------- Page 16 ----------------
    add_header_marker(doc)
    add_para(doc, '수기 가동, 슬러지 제거작업 등)에는 계속적으로 환기를 실시한다.', size=10)
    doc.add_page_break()

    # ---------------- Page 17 : 보호구 종류, 비상연락체계 ----------------
    add_header_marker(doc)
    add_section_title(doc, '작업 시 착용하여야 할 보호구의 종류', size=14)
    
    add_para(doc, '1. 밀폐공간 작업 시 기본 보호구 : 안전화, 안전모, 보안경, 그네식 안전대, 방진/방독마스크', size=10, bold=True, space_after=4)
    add_para(doc, '2. 추가 보호구 : (질소 충전 또는 산소농도 부족 작업시) 공기호흡기, 송기마스크,', size=10, bold=True, space_after=2)
    add_para(doc, '                안전블럭, 전동윈치, ELSA(Emergency Life Support Appratus)', size=10, bold=True, space_after=6)
    add_para(doc, ' ‑ 밀폐공간에 입조하는 작업자가 안전블럭과 전동윈치를 사용하는 경우, 체결 방법은 다음과 같음', size=10, space_after=2)
    add_para(doc, '     ∙ 안전블럭은 1줄걸이 죔줄을 안전그네 등 쪽 위에 있는 D링에 체결한다.', size=10, space_after=2)
    add_para(doc, '     ∙ 전동윈치는 2줄걸이 죔줄을 안전그네 가슴 쪽 양편에 있는 D링에 체결한다.', size=10, space_after=6)
    add_para(doc, ' ‑ 밀폐공간 입조 작업 시 비상대피용 공기호흡기 ELSA(Emergency Life Support Appratus)를 비치하는', size=10, space_after=2)
    add_para(doc, '   위치는 다음과 같이 한다.', size=10, space_after=2)
    add_para(doc, '     ∙ 밀폐공간이 질소분위기일 때 : 용기 바깥 출입구와 가까운 신선한 공기가 있는 장소', size=10, space_after=2)
    add_para(doc, '     ∙ 밀폐공간이 공기분위기일 때 : 용기 내 작업자들이 작업하는 장소와 가깝고 안전한 장소', size=10, space_after=10)

    add_section_title(doc, '비상연락체계', size=14)
    add_para(doc, '<응급처치 시 관찰사항>', size=11, bold=True, space_after=4)
    add_para(doc, '응급처치 시에는 다음의 사항을 주의 깊게 관찰하고 그 내용을 담당자에게 정확히 전달', size=10, space_after=6)
    for line in [
        '○ 의식이 있는지 확인한다.',
...
company_name = st.sidebar.text_input("회사명", "협력사명")
...
edu_date = st.sidebar.text_input("교육일정", "2025.01.15 08:00~09:00")
instructor = st.sidebar.text_input("강사", "안전관리자 김안전")
volume = st.sidebar.number_input("작업장소 용적 (m³)", value=284.5)
fan_model = st.sidebar.text_input("환기팬 모델명", "SMP-20")
fan_capacity = st.sidebar.number_input("환기팬 용량 (m³/h)", value=1800, step=100)
fan_count = st.sidebar.number_input("환기팬 수량 (EA)", value=3, step=1)

st.title("📄 밀폐공간 작업 프로그램 자동 작성기")
st.markdown("""
**원본 양식 서식(맑은 고딕, 정렬, 표 스타일, 여백)을 정교하게 반영한 버전입니다.**  
좌측에 정보를 입력 후 다운로드 버튼을 눌러 워드 파일을 받으세요.
""")
